...
public class MedicoController {
	...
	// salvar medico
	@PostMapping({"/salvar"})
	//Obtemos o email do usuário pelo AuthenticationPrincipal(este recebe um User, o User é referente ao UserDetails(objeto que  contém os dados da sessão do spring security)
	//para inserir um objeto médico e inserir na tabela médico
	public String salvar(Medico medico, RedirectAttributes attr, @AuthenticationPrincipal User user) {
		//insert pelo login de médico
		if (medico.hasNotId() && medico.getUsuario().hasNotId()) {
			//buscando o médico pelo email do usuário logado na aplicação
